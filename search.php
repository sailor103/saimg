...
//每页显示数据    
$num=11;
$sql_search="select * from `filelist` where `filename` like '%" . $ser . "%'";         
$total=mysql_num_rows(mysql_query($sql_search)); 
...
$offset=($page-1)*$num;         
//获取相应页数所需要显示的数据
$info=mysql_query( $sql_search . " order by `diskname` DESC limit $offset,$num");   
while($it=mysql_fetch_array($info))
{
   $tem[]=array('http://'.$it['host'].'/pic/'.$it['diskname'],$it['filename'],$it['diskname']);
}                                                              
//分页信息
$pre=$page-1;
$nex=$page+1;
...
if($pre>=1)
{
	$prepage="<a style=\"float:left;\" href=\"search.php?page=$pre&search=$ser\">上一页</a>";
	echo $prepage;
}
if($nex<=$pagenum)
{
	$nexpage="<a style=\"float:right;\" href=\"search.php?page=$nex&search=$ser\">下一页</a>";
	echo $nexpage;
}
?>
</div>
